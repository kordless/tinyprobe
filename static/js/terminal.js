...
var TerminalShell = {
	...
	process: function(terminal, cmd) {
		try {
			...
			var cmd_text = cmd.substr(cmd.indexOf(" ") + 1);
			if (this.commands.hasOwnProperty(cmd_name)) {
				cmd_args.unshift(terminal);
				this.commands[cmd_name].apply(this, cmd_args);
			} else {
				if (!(this.fallback && this.fallback(terminal, cmd_name, cmd_args, cmd_text))) {
...
var Terminal = {
	...
	jumpToBottom: function() {
		var scroll = $('#screen').prop('scrollHeight');
		$('#screen').animate({scrollTop: scroll}, this.config.scrollSpeed,'linear');
	},
	...
	scrollPage: function(num) {
		var scroll = $('#screen').scrollTop() + num * ($('#screen').height() * .75);
		$('#screen').animate({scrollTop: scroll}, this.config.scrollSpeed, 'linear');
	},
